...
如果你局限于普通按钮和其他常见控件的简单灰色外观，Silverlight 应用将会是单调乏味的一堆。幸运的是，Silverlight 有几个特性可以让您在基本元素中注入一些风格，并标准化应用的视觉外观。在这一章中，你将学习两个最重要的:风格和行为。
