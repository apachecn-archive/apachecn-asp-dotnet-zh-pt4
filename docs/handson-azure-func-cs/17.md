# 17.将认知能力添加到 Azure 功能中

人工智能已经成为现代应用开发的重要组成部分。它在人们能想到的每个领域都留下了印记。几乎每个上市的应用都有一个智能解决方案。从简单的聊天应用到虚拟助手的 app 都有认知能力。

在前面的章节中，我们讨论了开发和部署 Azure 功能的方法。您通过构建应用来解决各种用例，探索了 Azure 功能的不同绑定和触发器。有了前几章收集的知识，你就可以使用 Azure 函数构建无服务器解决方案了。在这一章中，您将通过向 Azure 函数添加认知能力来构建智能无服务器解决方案，从而为您的武器库增加一项技能。
