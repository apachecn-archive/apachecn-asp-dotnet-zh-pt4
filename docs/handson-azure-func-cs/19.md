...
在本节中，您将创建一个 Logic Apps 解决方案来从请求负载中获取文件名和文件内容。在 Azure 功能的帮助下加密文件内容后，您的 logic 应用程序将创建一个文件并将其存储在您的 Google Drive 中。你将使用凯撒密码作为你的加密算法。在本节结束时，您将能够使用 Google Drive 连接器，并探索将 Azure 功能集成到您的 Logic Apps 工作流中的方法。
