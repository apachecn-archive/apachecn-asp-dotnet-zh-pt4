...
构造函数的第一个依赖项是清单 10-3 中所示的`IUserService`，它为用户服务提供了签名。这些服务提供用户的创建、用户登录的验证以及从 ID 中检索用户名。HUB在这个接口中只使用`GetUserNameFromId`方法。
...
有时我们没有到HUB的连接，或者想要创建一个，但是我们仍然需要从应用访问数据。我们不需要每次都创建一个HUB连接，而是需要数据来为一些请求保持HUB连接打开。我们可以使用 Web API 端点来进行数据访问。
...
接下来是SignalR`DefaultDependencyResolver`的覆盖。我们使用这些方法来尝试使用 Unity 容器进行解析或注册。

在 10-59 清单中的 resolve 方法`GetService`和`GetServices`中，没有一种友好的方式来查看 Unity 是否可以解析一个类型，因此必须使用一个`try-catch`语句，如果解析失败，就退回到基类。

列出 10-59。UnityDependencyResolver SignalR程序默认值 DependencyResolver 覆盖
...
清单 10-96。Redis 配置用于 StartUp.cs 中的SignalR
...
我们希望你喜欢阅读这本书，现在有一个坚实的了解SignalR。我们祝你在未来的SignalR工程中好运。