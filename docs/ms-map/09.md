...
所有 Bing 地图控件都需要某种类型的认证令牌，以便微软可以跨应用跟踪地图使用情况，并适当地计量使用情况。不幸的是，使用地图控件为 Windows Phone 8 应用获取认证令牌的过程与使用 Bing 地图控件为 Web、WPF 或 Windows 8 获取认证令牌的过程不同。令人烦恼的是，您从用于将 Windows Phone 应用提交到 Windows Phone 商店的界面中获取认证令牌，并且在您的应用中以不同的方式设置它。要获得您的令牌:
