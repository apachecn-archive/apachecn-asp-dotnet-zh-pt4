...
![image](img/sq.jpg) **注** 像 AJAX 控件一样，Bing Maps for WPF 控件需要连接到互联网才能获得其地图切片。即使运行应用程序的计算机没有连接到 Internet，您也可以在 WPF 应用程序中使用该控件，但是如果没有 Internet 连接，它将无法下载或渲染地图！
...
4.  运行应用程序并按下按钮。你会在山景城看到红岩的一小段动画([图 7-2](#Fig2) )。
