...
![images](img/square.jpg) **注意**如果您以前使用过 ASP.NET WebForm，您会知道，由于 ASP.NET WebForm模型的架构，它被限制为每页一个表单。ASP.NET 的网页却不是这样。有了 ASP.NET 网页，在同一个 CSHTML 页面中可以完全接受多个非嵌套的表单。
