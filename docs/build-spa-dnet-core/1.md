...
使用 ASP.NET Web 窗体，方法是在窗体上拖放控件，设置它们的属性，并在事件处理程序中编写代码。WebForm引擎为您生成了 HTML 和 JavaScript。只有使用 ASP.NET MVC，开发人员才能最终控制他们的 HTML 和 JavaScript，使用 jQuery 及其插件进行主要的应用程序开发活动，如验证。还可以通过 ASP.NET MVC 使用 HTML 助手和数据注释来为 jQuery 插件生成正确的配置(jQuery 非介入式验证)。
