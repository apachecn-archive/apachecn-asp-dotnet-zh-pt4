...
![image](images/sq.jpg) **注**Damien Foggon 的书*ASP.NET 2.0 数据库入门:从新手到专业人士*(Apress，2006 年)从构建数据驱动网站的角度对关系数据库进行了很好的介绍。
...
![image](images/sq.jpg) **注意**微软 SQL Server 2012 有多个版本，就本书而言，所有提及微软 SQL Server 2012 的地方都是指企业版。
...
![image](images/sq.jpg) **注** *分片*是一种数据库架构，通过单独保存数据行来实现水平分区。这不同于规范化或垂直拆分。每个分区可以存储在不同的物理机器上。
...
![9781430243809_Fig02-01.jpg](images/9781430243809_Fig02-01.jpg)
...
![image](images/sq.jpg) **注意**列存储索引并不适用于所有地方。事实上，它们会降低某些类型的查询(如外部连接)的性能。
...
![image](images/sq.jpg) **注意**资源治理仅在微软 SQL Server 2012 企业版中可用。
...
![9781430243809_Fig02-02.jpg](images/9781430243809_Fig02-02.jpg)
...
![image](images/sq.jpg) **注意**在创建包含数据库之前，必须在数据库服务器实例上启用包含数据库选项。使用`sp_configure`程序启用包含的数据库选项。
...
![image](images/sq.jpg) **注**空间数据库针对地理空间数据的存储和检索进行了优化。它使用空间索引进行优化的空间查询。PostGIS 是地理空间数据库的一个例子。
...
![image](images/sq.jpg) **注**几何数据类型基于开放地理空间联盟标准，地理数据类型基于 WGS 84 坐标系。

SQL Server 2012 中的空间数据类型公开了几个简单和集合实例类型，如图 2-3 中的[所示。](#Fig3)

![9781430243809_Fig02-03.jpg](images/9781430243809_Fig02-03.jpg)
...
3.  Then, insert some sample spatial data to test the output.
    ...
    ![image](images/sq.jpg) **注意**空间数据类型实例使用一个*空间参考标识符(SRID)* 。SRID 表示用于平面或椭球面制图的空间参考系统。
...
4.  To view the results, run a `SELECT` query on the table. Microsoft SQL Server 2012 introduces a new result window called s*patial results*. You can use the spatial results to see the output plotted in the spatial graph, as shown in [Figure 2-4](#Fig4).
    ...
    ![9781430243809_Fig02-04.jpg](images/9781430243809_Fig02-04.jpg)
...
![image](images/sq.jpg) **警告**对非地理空间列应用空间索引会产生异常。
...
![image](images/sq.jpg) **注意**空间索引只能应用于带有聚集主键的表上。

也可以使用 SQL Server 2012 Management Studio 中的表设计器创建空间索引，如图[图 2-5](#Fig5) 所示。

![9781430243809_Fig02-05.jpg](images/9781430243809_Fig02-05.jpg)
...
![image](images/sq.jpg) **注意**空间聚合操作不适用于客户端库。
...
![9781430243809_Fig02-06.jpg](images/9781430243809_Fig02-06.jpg)
...
![image](images/sq.jpg) **注意**与`IDENTITY`不同，后者仅在插入一行时生成下一个值，而`SEQUENCE`可用于预取下一个值，而无需插入任何数据。
...
![image](images/sq.jpg) **注意**不能创建以任何特定数据库名为前缀的序列。它必须在模式上创建。您可以授予/撤销在特定模式上创建序列的权限。
...
![9781430243809_Fig02-07.jpg](images/9781430243809_Fig02-07.jpg)

[图 2-7。](#_Fig7)显示序列对象属性

![image](images/sq.jpg) **注意**如果您使用`CACHE`选项，您可以期待更好的性能。但是，在数据库突然出现故障的情况下，缓存值可能会丢失，并且可能会导致序列中断。
...
![image](images/sq.jpg) **注意**`NO CYCLE`选项将在达到最大值时阻止重启。
...
![image](images/sq.jpg) **注意**`NON_TRANSACTED`选项对于允许对文件共享的非事务性访问非常重要。
...
![9781430243809_Fig02-08.jpg](images/9781430243809_Fig02-08.jpg)

[图 2-8。](#_Fig8)电源视图设计器

您可以使用设计器通过将项从右侧拖动到设计图面上来构建数据可视化效果。您还可以从“文件”菜单中选择将报告保存为 PowerPoint 幻灯片。

![image](images/sq.jpg) **注意**电源视图不是为了取代报表生成器而设计的。它与报表生成器共存，主要面向希望在没有 DBA 或开发人员干预的情况下按需构建报表的高级用户。
...
![9781430243809_Fig02-11.jpg](images/9781430243809_Fig02-11.jpg)
...
![image](images/sq.jpg) **注意**在这一点上，并不是本地版本的所有操作在 SQL Azure 中都是可行的。在迁移到 SQL Azure 之前，请确保充分理解这些限制。
...
2.  Click the Database menu and select the server under your subscriptions. In the top navigation bar, you will see a host of options including the one to create a new database, as shown in [Figure 2-12](#Fig12).
    ...
    ![9781430243809_Fig02-12.jpg](images/9781430243809_Fig02-12.jpg)
...
4.  Select the created database from the left navigation pane and click `Manage`. It will launch the management portal.
    ...
    ![image](images/sq.jpg) **警告**为了登录到 SQL Azure 实例，请确保在 Azure 服务器防火墙规则下添加您的 IP 地址。
...
5.  After successfully signing into the portal, you can start creating the artifacts for the database. Also, use the query editor to run T-SQL queries. [Figure 2-13](#Fig13) illustrates managing the SQL Azure database through the management portal.
    ...
    ![9781430243809_Fig02-13.jpg](images/9781430243809_Fig02-13.jpg)

    [图 2-13。](#_Fig13)通过管理门户管理 SQL Azure 数据库

    ![image](images/sq.jpg) **注意**除了管理门户，您还可以使用 SQL Server Management Studio 来访问 SQL Azure 数据库，就像访问任何其他本地数据库一样。
...
![9781430243809_Fig02-14.jpg](images/9781430243809_Fig02-14.jpg)
