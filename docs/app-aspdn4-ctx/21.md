# 二十一、WebForm Triathlon 应用

在本章中，我们将使用它来重新创建我们在本书第二部分中看到的 triathlon 应用，从而完成我们的 WebForm 之旅。在这样做的时候，我们可以看到前面章节中的一些主题被具体应用并组合起来创建一个应用(尽管是一个简单的应用)。提醒一下，[图 21-1](#fig_21_1) 显示了 triathlon 应用的主页面。

![images](img/2101.jpg)

***图 21-1。**本书第二部分的铁人三项网络应用*

在我们构建应用时，有几点需要注意。首先，我故意不使用 jQuery，尽管我在前面的章节中敦促您利用它的特性。我这样做是因为 jQuery 对 web 应用生成的 HTML 进行操作，当考虑 WebForm 时，我们会考虑生成 HTML 的工具。添加 jQuery 可能会成为一个更好的应用，但却是一个更差的例子。

要注意的第二点是，使用 WebForm 控件有许多不同的方式。不仅控件具有重叠的功能和特性，而且将 WebForm 控件与 HTML 控件和静态 HTML 混合的能力为解决每个问题提供了广泛的方法。在这一章中，您将看到的方法是我的个人偏好(喜欢一种控件胜过另一种，或者我喜欢直接使用 LINQ)和我想要演示的技术(比如用 DataList 控件显示复杂的模板)的混合。你不应该把这些技术当成经典。相反，使用它们作为开发您自己的风格、偏好您自己的组件以及以最适合您的项目的方式获取和处理数据的起点。

### 创建项目

我们将从创建项目和设置基础开始。使用 Visual Studio 空 ASP.NET Web 应用模板创建一个名为 TriathlonApp 的项目。
...
我们将使用一个母版页来为应用中的页面提供一致的外观。你可以在第 9 章中了解更多关于母版页及其工作方式的信息。向应用添加一个新的母版页，并将其命名为`Site.master`。

现在我们已经创建了母版页，我们可以创建将使用它的页面。使用使用母版页模板的 WebForm 来创建名为`Events.aspx`、`EventEditor.aspx`、`Calculator.aspx`和`Performance.aspx`的页面。选择`Site.master`作为所有这些 WebForm 页面的母版页。
...
这个项目的母版页中有三个元素:页面顶部的静态图像横幅、允许用户选择应用中不同页面的选项卡，以及页面底部显示总计的页脚。我们将从最简单的开始，按照复杂程度来解决它们。
...
            **<asp:Image ID="Image1" runat="server" ImageUrl=img/triathlon.png" />**
...
横幅本身包含一个图像控件。通常，我不会使用该控件，除非我需要在代码隐藏类中以编程方式更改图像的某些方面，但由于我们正在查看 WebForm 和 WebForm 控件，我想我应该在这里包含它。

我还添加了一些注释来显示我将在哪里添加标签和页脚的标记。这是因为当我们添加新元素时，我不必复制整个标记。

#### 添加页脚

页脚是另一个简单的添加。它只显示事件的总数、行驶的总距离和事件时间的总和。除了为应用提供一些视觉一致性和修饰之外，它没有任何价值。清单 21-3 显示了支持页脚的标记。
...
`<%@ Master Language="C#" AutoEventWireup="true" CodeBehind="Site.master.cs"![images](img/U002.jpg)
 Inherits="TriathlonApp.Site" %>
...
                case "/Events.aspx":
                    EventButton.ImageUrl = img/btn-on-event.png";
                    break;
                case "/Performance.aspx":
                    PerfButton.ImageUrl = img/btn-on-perf.png";
                    break;
                case "/Calculator.aspx":
                    CalcButton.ImageUrl = img/btn-on-calc.png";
                    break;
            }
        }
...
当用户控件发出它的事件时，这个方法被调用，我简单地从`TabSelectedEventArgs`对象中获取值，并使用它将用户重定向到适当的页面。我可以很容易地在用户控件本身中完成这个简单的任务，但是我喜欢这种分离，它提供了一个很好的例子，说明如何将用户控件集成到 WebForm 应用的其他部分。
...
现在我们已经有了母版页，我们可以开始填充构成应用主干的各个页面，从`Event.aspx`开始。此页面显示数据库中的事件列表以及排名信息。可以根据事件类型过滤事件。有一个添加新事件的按钮，每个事件都有编辑或删除记录的按钮。清单 21-10 显示了我为这个页面添加的标记。只是一些基本的 CSS。
...
                runat="server" ImageUrl=img/btn-edit-off.png" Height="25"
...
                runat="server" ImageUrl=img/btn-delete-off.png" Height="25"
...
![images](img/square.jpg) **提示**您会注意到我没有在 DataList 控件中添加分页。这是为了简单起见，因为我们重新创建的应用也没有分页。你可以在第 20 章的[中看到富数据控件分页的例子。](20.html#ch20)
...
    ImageUrl=img/btn-delete-off.png" Height="25"
...
        ImageUrl=img/btn-add-event-off.png" Height="25"
...
![images](img/square.jpg) **注意**为了保持例子的简单，我省略了表单控件的任何验证。我不建议你在真实项目中这样做。参见[第 17 章](17.html#ch17)了解网络表单验证控件的细节和使用示例。
...
这个页面特别展示了 WebForm 在处理数据方面的优势。我们只是使用数据源控件选择了我们想要的数据，设置了以不同方式显示数据的模板，并定义了一些按钮控件来触发操作。我们根本不需要处理加载、更新、删除或添加记录。一切都为我们处理好了。

### 构建绩效页面

我们已经处理了这个应用的所有复杂部分。剩下的只是一些与第二部分的实现非常相似的页面。性能页面比较简单；您可以在清单 21-17 中看到这个标记。我使用数据源控件将数据库中运动员的名字填充到 DropDownList 中，并使用常规 HTML `table`中的单个标签控件来显示数据值。这是我经常使用的一种方法，当我需要在代码隐藏类中设置一个值或者当我想要接收和处理事件时，对不改变的元素和 WebForm 控件使用常规 HTML。
...
我不打算深入研究这段代码的细节。这个类中没有你在前面章节中没有见过的东西，因为我们不需要任何特殊的 WebForm 特性来处理或显示数据。您可以在[图 21-9](#fig_21_9) 中看到浏览器显示的已完成的`Performance.aspx`页面。
...
剩下的页面甚至更简单。然而，为了多样化，我使用了一个 WebForm 表格控件，如清单 21-19 所示。
...
                ImageUrl=img/btn-calculate-off.png" Height="25"/>
        </asp:TableCell>
    </asp:TableFooterRow>
</asp:Table>
...
有了这个类，我们就完成了这个应用，增加了前面章节中提到的一些关键的 WebForm 功能。在[图 21-10](#fig_21_10) 中可以看到完成的`Calculator.aspx`页面。
...
在本章中，我们使用 WebForm 控件和特性实现了 triathlon 应用。在这个简单的应用中，我们看到了 WebForm 的优点和缺点。优势在于我们创建包含数据的页面的速度和简单性，以及我们操作数据来编辑、删除或向底层数据库添加记录的容易程度。我们还将 WebForm 控件与静态 HTML 结合起来，这是我经常做的事情。

缺点也同样明显，比如使 WebForm 控件按要求运行所需的小调整和小技巧，以及数据源控件在复杂数据操作方面的局限性。然而，总的来说，WebForm 控件做得很好，尽管可能需要一些尝试和错误来让控件按照我们想要的那样工作，但是考虑到结果的丰富性和灵活性，花费的时间是值得的。

我回到了我打开这本书的这一部分时的主题。Web Forms 光滑、灵活、功能丰富，如果用在合适的项目上，可以产生很好的效果。根据你所面临的情况，抛开 MVC 框架的光芒，拥抱强大而有用的 WebForm 是值得的。