...
大多数 web 应用使用 HTML 表单从用户那里收集某种数据。这包括从简单数据(如姓名和密码)到对业务流程至关重要的复杂数据。作为 web 应用开发人员，我们负责获取用户提供的数据并理解这些数据。用户也是人，他们会犯错误或误读指令。检查我们接收到的输入的过程被称为*验证*。本章描述了一组支持验证的 WebForm控件。
