...
您使用数据源作为应用程序中的数据。WebForm包括一些内置的数据源控件，使这一过程对于各种数据类型来说都很简单。您可以像处理 SQL 数据库中的对象一样轻松地处理在内存中创建的对象。我将介绍最有用的数据源控件，并演示如何在没有它们的情况下获得一些高级结果。
...
当我说向 WebForm应用程序添加数据是多么容易时，您会明白我的意思。诚然，本节中的示例非常简单，但是正如您将看到的，即使在处理更复杂的数据和控件时，基本模式也保持不变。
