...
在本章中，我向你展示了我们如何使用 MVC 框架的特性来构建 triathlon 应用。您已经看到了如何使用核心的 ASP.NET 特性、使用 WebForm控件以及现在使用 MVC 框架来创建这个应用。每一个都有非常不同的性质，给开发者带来不同的体验。我在使用 MVC 框架时的感觉是，必须忍受一个缓慢的开始(创建模型、存储库、布局，等等)，然后功能开始以越来越快和越来越容易的速度结合在一起。