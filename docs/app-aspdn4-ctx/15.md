...
***清单 15-3。**从 div 元素和 Web 表单面板控件呈现的 HTML】*
...
![images](img/square.jpg) **注意**bulleted list 控件也支持数据绑定，其中显示在列表中的项目是从数据源(如数据库)获得的。参见[第 18 章](18.html#ch18)和[第 19 章](19.html#ch19)了解在 Web 表单控件中使用数据的详细信息。
...
![images](img/square.jpg) **注意**你会很快意识到我不喜欢其中的一些控制。他们制作的 HTML 不够优雅，而且严重依赖内联脚本。如果您想在整个应用程序中使用 Web 窗体，那么这些控件会很有用。但是我相信，对于这些控件，有更简单、更优雅的方法可用，只要您愿意在 Web 表单抽象中找出漏洞，并使用 jQuery 之类的工具。
