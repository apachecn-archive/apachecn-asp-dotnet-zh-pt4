...
您可以通过启动应用，使用浏览器导航到`/Home/Today` URL，然后单击 Get Day 按钮来查看新路线的效果。客户端将向`/api/today/dayofweek` URL 发送一个请求，该请求将与我在清单 20-8 中定义的路由相匹配。路线模板将创建名为`controller`和`action`的路线数据变量，对应于变量段，值为`today`和`dayofweek`。行动方法选择过程将调用由`Today`控制器定义的`DayOfWeek`行动方法，这将产生图 [20-4](#Fig4) 。
