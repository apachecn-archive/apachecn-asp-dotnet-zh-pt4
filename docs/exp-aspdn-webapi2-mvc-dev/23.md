...
顾名思义，身份验证过滤器允许您确保动作方法仅由已经过身份验证的客户端调用。大多数 Web API 应用将使用 ASP.NET 身份平台来执行身份验证，您在第 6 章中看到了如何使用过滤器来应用 ASP.NET 身份。表 [23-11](#Tab11) 将认证和过滤器放在上下文中。
...
| 你应该什么时候使用它们？ | 当您希望将对一个或多个操作方法的访问限制为由应用已知的用户发出的请求时，请应用身份验证筛选器。 |
| 你需要知道什么？ | 只要请求包含有效凭据，身份验证过滤器将允许所有用户访问。如第 24 章所述，如果您想限制特定用户的访问，请使用授权过滤器。 |

### 准备身份验证

如果您的应用需要定制交付给不同用户的功能和内容，那么您需要某种方式来管理用户的身份和安全凭证。这通常是通过 ASP.NET 身份系统(或者它的前身，ASP.NET 会员)来处理的，但是除了我为 SportsStore 应用给出的描述之外，我不想深入讨论身份的细节，所以在本章中，我将定义一个用户管理器类，我将用它来使用静态数据对用户进行身份验证和授权。清单 23-11 显示了我添加到`Infrastructure`文件夹中的`StaticUserManager.cs`类文件的内容。
...
身份验证过滤器负责确定发出请求的用户的身份。Web API 不指定如何识别用户，这意味着身份验证过滤器可用于将任何用户管理系统集成到您的 Web 服务中。对于大多数应用来说，ASP.NET 身份系统是处理认证(和授权，我在第 24 章中描述过)的最佳方式，但即使如此，理解 Web API 认证过程是如何工作的也是有用的，尤其是当您在自己的项目中没有得到预期的结果时。
...
在 MVC 框架应用中，通常通过将客户端重定向到应用的登录 URL 来处理身份验证失败。这种技术不适用于 HTTP web 服务，在这种情况下，不能假定客户端能够解析 HTML 或提示用户输入凭据。Web 服务可以用来认证用户——正如我在第 24 章中演示的那样——但是失败的认证应该通过向客户端返回 401(未授权)响应来处理。
...
要测试过滤器，启动应用并使用浏览器导航到`/Home/Index` URL。首先，单击 Get All 或 Post 按钮，确保未经身份验证的请求得到正确处理。要检查身份验证过程，请单击“获取一个”按钮。
