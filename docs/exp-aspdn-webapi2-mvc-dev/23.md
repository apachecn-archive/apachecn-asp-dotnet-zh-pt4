...
我在第 20 章中提到了过滤器，这样我就可以描述`ApiController`类实现的分派过程。在这一章中，我将深入描述过滤器是如何工作的，并演示如何使用它们来为 Web API 应用添加横切关注点。表 [23-1](#Tab1) 总结了本章内容。
...
| 对应用中的所有操作方法应用过滤器。 | 定义全局过滤器。 | 9, 10 |
| 将用户身份与请求相关联。 | 创建认证过滤器。 | 11–13 |
...
过滤器将额外的逻辑注入到`ApiController`分派过程中，并提供一个简单而优雅的机制来实现在 MVC 模式中跨多个组件操作的特性，这被称为横切关注点。过滤器最常见的用途是应用认证和授权、处理错误以及测量性能。
...
| 你应该什么时候使用它们？ | 应该谨慎使用操作过滤器，并且仅用于执行不包含业务逻辑的任务、对数据模型进行操作或执行认证或授权(由其他过滤器类型处理)。 |
...
为了执行我的测量，我需要利用在动作方法执行之前和之后执行工作的机会。在执行之前，我需要启动一个计时器。在执行之后，我需要读取计时器的值并报告运行时间，这是通过向`HttpResponseMessage`对象添加一个标题并向 Visual Studio 输出窗口写入一条消息来完成的。
...
您可以通过启动应用、导航到`/Home/Index` URL 并依次单击 Get All、Get One 和 Post 按钮来测试动作过滤器。除了响应中的标头之外，操作筛选器还会将消息写入 Visual Studio 输出窗口，如下所示:
...
问题是属性集合中包含的状态数据在存储数据的组件和检索数据的组件之间产生了紧密耦合。在这种情况下，您必须在您的应用中使用这两个组件——这使得在没有弄清楚状态数据的私有含义并重新创建或替换它的情况下，很难创建单个接口的自定义实现。
...
我已经将过滤器直接应用于无参数的`Get`动作方法，而不是整个控制器。要执行测试，启动应用并使用浏览器导航到`/Home/Index` URL。单击“全部获取”按钮四次，您将在 Visual Studio 输出窗口中看到类似以下内容的消息:
...
要测试自定义操作选择器并获得过滤器管道的详细信息，请启动应用并使用浏览器导航到`/Home/Index` URL。单击“全部获取”按钮，您将在 Visual Studio 输出窗口中看到类似以下内容的消息:
...
要测试全局过滤器，启动应用并使用浏览器导航到`/Home/Index` URL。单击“全部获取”按钮，您将在 Visual Studio 输出窗口中看到类似以下内容的消息:
...
顾名思义，认证过滤器允许您确保动作方法仅由已经过认证的客户端调用。大多数 Web API 应用将使用 ASP.NET 身份平台来执行认证，您在第 6 章中看到了如何使用过滤器来应用 ASP.NET 身份。表 [23-11](#Tab11) 将认证和过滤器放在上下文中。

无论作用域如何，在调用任何其他类型的过滤器和操作方法之前，都会首先调用认证过滤器。当认证失败时，分派过程被短路，并且从认证过滤器产生一个`HttpResponseMessage`,而不调用任何其他过滤器——或者目标动作方法。
...
| 问题 | 回答 |
| --- | --- |
| 它们是什么？ | 认证过滤器负责检查 HTTP 请求并识别与之相关的用户身份。没有用户可以关联的请求被短路。 |
| 你应该什么时候使用它们？ | 当您希望将对一个或多个操作方法的访问限制为由应用已知的用户发出的请求时，请应用认证筛选器。 |
| 你需要知道什么？ | 只要请求包含有效凭据，认证过滤器将允许所有用户访问。如第 24 章所述，如果您想限制特定用户的访问，请使用授权过滤器。 |

### 准备认证

如果您的应用需要定制交付给不同用户的功能和内容，那么您需要某种方式来管理用户的身份和安全凭证。这通常是通过 ASP.NET 身份系统(或者它的前身，ASP.NET 会员)来处理的，但是除了我为 SportsStore 应用给出的描述之外，我不想深入讨论身份的细节，所以在本章中，我将定义一个用户管理器类，我将用它来使用静态数据对用户进行认证和授权。清单 23-11 显示了我添加到`Infrastructure`文件夹中的`StaticUserManager.cs`类文件的内容。
...
| 名字 | 描述 |
| --- | --- |
| `AuthenticationType` | 返回一个字符串，该字符串指定用于创建标识的认证类型 |
...
我已经将用户名、密码和角色硬编码到了`StaticUserManager`类中，这对于一个简单的例子来说很好，但在实际项目中会产生问题，因为这意味着每次有新用户或现有用户发生变化时，您都必须部署新版本的应用。

### 了解认证过滤器

认证过滤器负责确定发出请求的用户的身份。Web API 不指定如何识别用户，这意味着认证过滤器可用于将任何用户管理系统集成到您的 Web 服务中。对于大多数应用来说，ASP.NET 身份系统是处理认证(和授权，我在第 24 章中描述过)的最佳方式，但即使如此，理解 Web API 认证过程是如何工作的也是有用的，尤其是当您在自己的项目中没有得到预期的结果时。
...
在 MVC 框架应用中，通常通过将客户端重定向到应用的登录 URL 来处理认证失败。这种技术不适用于 HTTP web 服务，在这种情况下，不能假定客户端能够解析 HTML 或提示用户输入凭据。Web 服务可以用来认证用户——正如我在第 24 章中演示的那样——但是失败的认证应该通过向客户端返回 401(未授权)响应来处理。
...
我在本节中描述的示例认证过滤器使用基本的认证机制，该机制被定义为 HTTP 规范的一部分。它实现起来非常简单，但没有得到广泛应用，因为它有一些深刻的局限性，尤其是在没有 SSL 的情况下。
...
如果客户端发送一个没有`Authorization`头或凭证不正确(指定不存在的用户或无效密码)的请求，那么 web 服务将返回一个 401(未授权)响应，其中包含一个指定基本认证机制的`WWW-Authenticate`，如下所示:

`WWW-Authenticate: Basic`

基本认证要求客户端以易于解码的格式向 web 服务发送用户凭证和每个请求。如果您发现自己正在使用基本认证(我真诚地希望您不要这样做)，那么请确保您的所有请求都通过 SSL 处理。
...
这个过滤器依赖于基本的 HTTP 认证方案，除非使用 SSL，否则它完全不适合在实际项目中使用。使用这个示例只是为了理解认证过滤器机制是如何工作的，有关如何在实际项目中使用 ASP.NET 身份来处理用户认证的详细信息，请参见第 1 部分。
...
这堂课有很多内容，所以我将在接下来的章节中分解细节。当我深入讨论细节时，要记住的最重要的事情是认证过滤器负责从请求中识别用户，而不是从用户那里获取凭证。
...
*   如果请求通过了认证，则将`HttpAuthenticationContext.Principal`属性设置为`IPrincipal`接口的一个实现。
...
注意，在尝试验证请求之前，我显式地将`HttpAuthenticationContext.Principal`属性设置为`null`。如果 Web API 应用运行在传统的 ASP.NET 平台上，那么在调用`AuthenticateAsync`方法来表示来自 Windows 或其他地方的凭证时，`Principal`属性可能已经设置好了。可以定位并禁用其他来源的`IPrincipal`对象，但是我发现将`Principal`属性显式设置为`null`更加可靠。
...
第二个场景是认证成功，调用了`ChallengeAsync`方法，这样过滤器就可以修改`HttpResponse`对象，通常是为了更容易对后续请求进行认证。
...
认证过滤器的应用方式与任何其他过滤器一样，可以指定全局、控制器和操作范围。清单 23-13 显示了我如何将我的示例过滤器应用于`Products`控制器。

清单 23-13。在 ProductsController.cs 文件中应用认证筛选器
...
认证过滤器不会限制特定用户的访问——这是授权过滤器的工作，我在第 24 章中描述过。认证过滤器将向任何经过认证的请求授予访问权限，而不考虑与之关联的用户身份。

要测试过滤器，启动应用并使用浏览器导航到`/Home/Index` URL。首先，单击 Get All 或 Post 按钮，确保未经认证的请求得到正确处理。要检查认证过程，请单击“获取一个”按钮。
...
如果你查看 Visual Studio 输出窗口，你会看到我在本章前面创建的`PipelineActionSelector`类已经写出了请求的过滤器管道。对于以我应用了认证过滤器的`Get`方法为目标的请求，输出如下所示:
...
我使用 LINQ 按类型排列过滤器，当您启动应用，导航到`/Home/Index` URL，并单击 Get One 按钮时，它会在 Visual Studio 输出窗口中产生以下消息:
